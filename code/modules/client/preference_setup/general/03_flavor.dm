...
/datum/category_item/player_setup_item/physical/flavor/content(var/mob/user)
	. = list()
	. += "<b>Flavor:</b><br>"
	. += "<a href='?src=\ref[src];flavor_text=open'>Character flavor</a><br/>"
	. += "<a href='?src=\ref[src];flavour_text_robot=open'>Robot flavor</a><br/>"
...
/datum/category_item/player_setup_item/physical/flavor/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["flavor_text"] && href_list["flavor_text"] == "open")
		var/msg = sanitize(input_cp1251(usr,"Enter character's flavor.","Flavor Text", pref.flavor_text, "message"), extra = 0)
		if(CanUseTopic(user))
			if(msg)
		...
		switch(href_list["flavour_text_robot"])
			if("open")
			if("Default")
				var/msg = sanitize(input(usr,"Enter robot's flavor.","Flavour Text",html_decode(pref.flavour_texts_robot["Default"]), "message"), extra = 0)
				if(CanUseTopic(user))
